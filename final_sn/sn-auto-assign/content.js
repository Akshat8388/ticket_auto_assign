...
(async function () {
  ...
  if (pendingReturnToQueue && Date.now() - pendingReturnToQueue < 15000) {
    await chrome.storage.local.set({ pendingReturnToQueue: null });
  ...
  const assignedToField = queryVisible('input[id="sys_display.incident.assigned_to"]');
  if (assignedToField) {
  ...
  for (const row of rows) {
    ...
    if (isEmpty) {
      ...
      if (link && link.href) {
        ...
        const expectedNumber = link.textContent.trim();
        await chrome.storage.local.set({ pendingAssign: true, expectedTicketNumber: expectedNumber });
...
async function returnToQueue(delayMs) {
  await chrome.storage.local.set({ pendingReturnToQueue: Date.now() });
  setTimeout(async () => {
    const { queueListUrl } = await chrome.storage.local.get(["queueListUrl"]);
    window.location.href = queueListUrl || self.SN_DEFAULTS.queueListUrl;
  }, delayMs);
...
async function handleFormPage() {
  const cfg = await chrome.storage.local.get(["pendingAssign", "queueListUrl", "assigneeName", "expectedTicketNumber"]);
  ...
  const numberCheckField =
    queryVisible('input[id="sys_display.incident.number"]') ||
    queryVisible('input[id="sys_readonly.incident.number"]') ||
    queryVisible('input[id="incident.number"]');
  const visibleNumber = numberCheckField ? numberCheckField.value.trim() : null;

  if (cfg.expectedTicketNumber && visibleNumber && visibleNumber !== cfg.expectedTicketNumber) {
    await finish({
      __snAssign: "error",
      message: `Safety check failed: expected ${cfg.expectedTicketNumber} but page shows ${visibleNumber} - aborted without touching anything.`
    });
    return;
  }

  const input = queryVisible('input[id="sys_display.incident.assigned_to"]');
  if (!input) {
  ...
  const saveBtn = queryVisible("#sysverb_update_and_stay");
  if (!saveBtn) {
