...
async function load() {
  const cfg = await chrome.storage.local.get(["assigneeName", "pollMinutes", "running", "log", "ringing", "ringingNumber", "backendUrl"]);
  $("assigneeName").value = cfg.assigneeName || self.SN_DEFAULTS.assigneeName;
  $("backendUrl").value = cfg.backendUrl || self.SN_DEFAULTS.backendUrl;
  $("pollMinutes").value = cfg.pollMinutes || self.SN_DEFAULTS.pollMinutes;
  setRunningUI(!!cfg.running);
  renderLog(cfg.log || []);
  setRingingUI(!!cfg.ringing, cfg.ringingNumber);
}

function setRingingUI(ringing, number) {
  const btn = $("stopAlert");
  if (ringing) {
    btn.classList.remove("hidden");
    btn.textContent = `🔔 ${number ? number + " grabbed" : "Ticket grabbed"} — Stop Alert`;
  } else {
...
chrome.storage.onChanged.addListener((changes) => {
  ...
  if (changes.ringing || changes.ringingNumber) {
    chrome.storage.local.get(["ringing", "ringingNumber"], (cfg) => {
      setRingingUI(!!cfg.ringing, cfg.ringingNumber);
    });
  }
});
